...
class BaseTrainer(object):
    ...
    def save_image(self, path, data):
        ...
        if is_master() and vis_images is not None:
            # iterate over vis_images and repeat channels for single channel images
            for i, img in enumerate(vis_images):
                if img.size(0) == 1:
                    vis_images[i] = torch.cat([img, img, img], dim=0)
                    
                # vis_images = (vis_images + 1) / 2
                # do per item, only if the values are -1 to 1
                if vis_images[i].min() < 0:
                    vis_images[i] = (vis_images[i] + 1) / 2
                    vis_images[i] = vis_images[i]
