...
class Trainer(BaseTrainer):
    ...
    def _init_loss(self, cfg):
        ...
        if hasattr(cfg.trainer, 'masked_training'):
            if cfg.trainer.masked_training:
    ...
    def gen_forward(self, data):
        ...
        if self.masked_training:
            mask_images = data['images'] <= -1.
            net_G_output['fake_images'] = torch.where(mask_images, -1, net_G_output['fake_images'])
        ...
        # # check if all are normalized
        # print("images range", data['images'].max(), data['images'].min())
        # print("label range", data['label'].max(), data['label'].min())
        # print("generated range", net_G_output['fake_images'].max(), net_G_output['fake_images'].min())
    ...
    def dis_forward(self, data):
        ...
        if self.masked_training:
            mask_images = data['images'] <= -1.
            net_G_output['fake_images'] = torch.where(mask_images, data['images'], net_G_output['fake_images'])
    ...
    def _get_visualizations(self, data):
        ...
        with torch.no_grad():
            ...
            for key in labels.keys():
                # check if 3 channel image
                if labels[key].size(1) == 3:
                    # DEBUG
                    print(key, labels[key].min(), labels[key].max())
            ...
            if 'images' in data.keys():
                # DEBUG
                print("images", data['images'].min(), data['images'].max())
                if data['images'].size(1) == 3:
                    vis_images.append(data['images'])
                    vis_images.append(net_G_output['fake_images'])
                else:
                    vis_images.append(data['images'].expand(-1, 3, -1, -1))
                    vis_images.append(net_G_output['fake_images'].expand(-1, 3, -1, -1))
            ...
            if self.cfg.trainer.model_average_config.enabled:
                ...
                if net_G_model_average_output['fake_images'].size(1) == 3:
                    vis_images.append(net_G_model_average_output['fake_images'])
                else:
                    vis_images.append(net_G_model_average_output['fake_images'].expand(-1, 3, -1, -1))
        return vis_images
    ...
    def _resize_data(self, data):
        ...
        data['images'] = F.interpolate(
            data['images'], size=[sy, sx], mode='nearest')
        return data
