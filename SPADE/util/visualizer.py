                ...
                for label, image_numpy in visuals.items():
                    if isinstance(image_numpy, list):
                        for i, img in enumerate(image_numpy):
                            ...
                            self.tf.summary.image(f"{label}/{i}", [img], step=step)
                    else:
                        image_numpy = np.clip(image_numpy, 0, 255) / 255.0
        ...
        if self.use_html: # save images to a html file
            for label, image_numpy in visuals.items():
                if isinstance(image_numpy, list):
                    ...
                    if len(image_numpy.shape) >= 4 or (self.opt.output_nc == 1 and len(image_numpy.shape) == 3):
        ...
        if self.tf_log:
            with self.writer.as_default():
                for tag, value in errors.items():
                    self.tf.summary.scalar(tag.detach().cpu().numpy(), value.detach().cpu().numpy(), step=step)
                self.writer.flush()
