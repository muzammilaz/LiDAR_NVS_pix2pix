...
class Pix2pixDataset(BaseDataset):
    ...
    def get_paths(self, opt):
        ...
        if opt.mask_in_data:
            mask_dir = os.path.join(opt.dataroot, f'{phase}_masks')
    ...
    def __getitem__(self, index):
        ...
        transform_image = get_transform(self.opt, params, nc = self.opt.output_nc)
        image_tensor = transform_image(image)

        # mask image read and process as single channel
        if self.opt.mask_in_data:
            mask_path = self.mask_paths[index]
            mask = Image.open(mask_path)
            transform_mask = get_transform(self.opt, params, method=Image.NEAREST, normalize=False, toTensor=True, nc = 1)
